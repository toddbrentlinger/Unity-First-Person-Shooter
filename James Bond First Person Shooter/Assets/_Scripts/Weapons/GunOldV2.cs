...
public class GunOldV2 : MonoBehaviour {
    ...
    [SerializeField]
    private float m_reloadDurationFactor = 1.0f; // multiplied to length of reload animation to adjust duration of animation
    [SerializeField]
    private float m_weaponRange = 50f; // Length of ray cast to test against colliders
    [SerializeField]
    private int m_gunDamage = 1; // How much damage is applied to target health. Interacts with Health script on target
    [SerializeField]
    private float m_bulletForce = 2.0f; // Force of bullet on rigidbodies
    [SerializeField]
    private float m_bulletScatter = 0; // Value of zero is NO scatter, perfect accuracy

    [Header("Audio")]
    [SerializeField]
    private AudioClip m_fireWeaponAudioClip; // AudioClip of weapon firing
    [SerializeField]
    private AudioClip m_emptyChamberAudioClip; // AudoClip of empty chamber click
    [SerializeField]
    private AudioClip m_weaponReloadAudioClip; // AudioClip of weapon reloading
